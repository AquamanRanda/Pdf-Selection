...
(function(exports) {
  ...
  function getDocumentOpacity() {
    var leftDocOpacity = 1;
    var rightDocOpacity = 1;

    if (exports.DiffOverlaySlider.isEnabled()) {
      var value = exports.DiffOverlaySlider.getValue();
      leftDocOpacity = 1 - value;
      rightDocOpacity = value;
    }

    return {
      leftDocOpacity: leftDocOpacity,
      rightDocOpacity: rightDocOpacity,
    };
  }

  function allowViewPortRendering(allow) {
    var midPanelInstance = instances[PANEL_IDS.MID_PANEL].instance;
    var docViewer = midPanelInstance.docViewer;
    if (allow) {
      docViewer.setViewportRenderMode(true);
      midPanelInstance.setMaxZoomLevel('9999%');
    } else {
      var wasInViewportRender = docViewer.isInViewportRenderMode();
  ...
  function renderMidPanelOverlay(firstDocIndex, secondDocIndex, middleSecondDocToDraw, desiredWidth, desiredHeight, firstDocCanvasMatrix, secondDocCanvasMatrix, shouldDiff) {
    ...
    var diffCanvas = function(firstDocCanvas) {
      ...
      var result;
      var documentOpacity = getDocumentOpacity();
      var firstDocOpacity = documentOpacity.leftDocOpacity;
      var secondDocOpacity = documentOpacity.rightDocOpacity;
      if (shouldDiff) {
  ...
  function setupViewer(item) {
    return new Promise(function(resolve) {
      ...
      WebViewer(
        ...
        instance.annotManager.on('annotationChanged', function(annotations, action) {
          if (action === 'add') {
            var internalAnnotations = annotations.filter(function(annot) {
              return annot.getInternal();
            });
  ...
  function initializeViewers(array, callback) {
    ...
    Promise.all(array.map(setupViewer)).then(function() {
      ...
      var overlaySliderCheckboxToggle = {
        type: 'customElement',
        render: function() {
          var span = document.createElement('span');
          span.style = 'display: flex; align-items: center;';
  ...
  function initialize(firstPdfRelPath, secondPdfRelPath) {
    ...
    exports.DiffAlignmentTool.initializeDiffAlignmentToolHandlers(instances[PANEL_IDS.LEFT_PANEL].instance, instances[PANEL_IDS.RIGHT_PANEL].instance, alignLineSegments);
  }
  ...
  document.getElementById('download-middle-panel-image').onclick = function(e) {
    e.preventDefault();
    var documentContainer = instances[PANEL_IDS.MID_PANEL].documentContainer;
    var canvasElement = documentContainer.querySelector('.canvasOverlay');
